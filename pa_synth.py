# -*- coding: utf-8 -*-
"""③ 음운인식(합성) — 닫힌집합(closed-set) 검증 모델.

매뉴얼 원칙: "목표 단어 후보 안에서 인식 → 말한 단어가 목표와 같은지 판정".
낱말/단락 읽기의 '열린 전사'와 패러다임이 다르다. 후보가 16개로 닫혀 있으므로
자유 디코딩(argmax) 대신 **후보 제약**을 건다:

  각 후보 단어의 음소열을 오디오에 CTC forced-align → Viterbi 경로 log-우도 계산
  → 최우도 후보 선택 → 목표와 일치하면 정답(pass), 아니면 오답(fail).

짧은 무의미 음절(따·규…2음소)은 자유 디코딩이면 오인식이 잦지만, 후보 제약을
걸면 "16개 중 무엇이냐"만 풀면 되므로 훨씬 강건하다. 그래서 이 과제는 낱말/단락
음향모델을 그대로 재사용하되 디코딩만 제약하며, 별도 파인튜닝 없이 baseline으로 간다.

두 경로:
  verify(wav)      : 실제 오디오 → forced-align 우도로 후보 선택(모델 필요).
  verify_dry(phones): slplab 음소열 문자열 → 후보 중 음소 최근접 선택(모델 없이 로직 점검).
"""
...
# 잠정값. 임상 채점본과 대조해 보정 필요(매뉴얼 §채점 캘리브레이션).
GOP_OK = -3.0
