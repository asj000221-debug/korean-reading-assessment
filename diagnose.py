"""진단·처방 오케스트레이터 — 채점 결과를 임상 리포트로.

기존 파이프라인(asr→g2p→align→scoring)은 '정확도 %'에서 멈춘다. 여기서 그 위에
임상 계층을 얹는다: 채점 결과 → 임상 오류 프로파일(error_taxonomy) → 약점 스킬 +
스킬별 정확도(evidence) → 발달 배치·다음 목표(skill_map) → 개선방향 처방.
채점기를 진단·처방 엔진으로 묶는 결합부다.
"""
