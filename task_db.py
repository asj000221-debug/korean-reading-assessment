...
표준화 검사 매뉴얼의 구조(3과제·타입 규칙·산출 구분)를 따르되,
문항 자체는 검사 보안을 위해 공개용 목업으로 교체했다.

정답 DB의 단일 진실 소스(single source of truth).
세 개의 ASR 과제(①낱말읽기 ②단락읽기 ③음운인식-합성)만 담는다.
(낱자소리대응·표기인식·연산·계수·작업기억(터치)·빠른자동이름대기는 ASR 불필요 → 범위 밖.)

각 낱말 item:
  word : 화면 제시 철자(원본)
  type : 채점 엄격도 — 'transparent'|'phonrule'|'nonword'
  pron : 채점 기준 발음(한글). 매뉴얼 원칙대로 "발음 규칙으로 생성"(g2p 결과).
         무의미낱말도 사전이 없으므로 규칙형을 기준으로 둔다(예: 낱발→낟빨).

타입 규칙(매뉴얼 §채점 로직 + KOLRA 차등 엄격도):
  - 의미낱말 중 g2p(word)==word  → transparent (받침 조음미숙 관대)
  - 의미낱말 중 g2p(word)!=word  → phonrule   (음운규칙 적용 실패도 오류; 엄격)
  - 무의미낱말 전부              → nonword    (맥락추측 불가, 가장 엄격)

pron 값은 g2pkk로 사전 계산해 박아둔다(mecab 없는 배포환경에서도 동작).
"""
