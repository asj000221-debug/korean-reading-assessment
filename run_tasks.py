# -*- coding: utf-8 -*-
"""매뉴얼 3개 ASR 과제 통합 러너 — task_db(정답 DB) 연결.

과제별 '모델 / 인식단위 / 채점'을 하나의 진입점으로 묶는다:

  ① 낱말읽기  : slplab 음소-CTC 열린전사 → 단어별 자모 diff → 유형별 엄격도 채점
                (의미: transparent/phonrule, 무의미: nonword. 무의미 최엄격.)
  ② 단락읽기  : 같은 음향모델 재사용 → P1 어절 정렬 → 어절 정확도
  ③ 음운인식  : 닫힌집합(16후보) 검증 → pa_synth (별도 모델 아님, 디코딩 제약)

공용 음향모델은 낱말/단락/음운인식이 공유한다(파인튜닝은 낱말읽기 하나에 집중).
각 과제는 --wav(실제 오디오) 또는 --dry(음소열 직접 입력)로 실행. dry는 모델 없이
채점 로직만 점검한다.
"""
