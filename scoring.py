"""STEP 4 — 문항 유형별 채점 룰 (핵심 비즈니스 로직, 학습 0).

STEP 3의 raw 오류 목록(Error)에 문항 유형 규칙을 적용해 최종 점수를 산출한다.
전부 조건문이다. 학습 없음.

문항 유형(§2-4):
  nonword     : 모든 자모 차이를 오류로(가장 엄격). 맥락 추측 불가 → 순수 해독력.
  phonrule    : 음운규칙 적용 실패도 오류로 카운트(이게 측정 대상). 엄격.
  transparent : 종성(받침) 단순 조음 오류는 감점 제외(가중치 0).
                단, 받침이 바뀌어 '다른 실단어'가 되면 오류로 카운트.

출력(WordScore): 정/오, 오류 태그 목록, 정확도(0~1).
"""
