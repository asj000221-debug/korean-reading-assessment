"""GOP(Goodness of Pronunciation) — 발음 신뢰도.

argmax가 비슷한 음소로 헛디뎌도(시→지), 모델이 '기대 음소'에 준 확률을
직접 보면 그 소리를 실제로 냈는지 알 수 있다.

기대 음소열을 음향에 CTC forced-align 하고, 각 기대 음소의 프레임별
log-posterior 평균을 GOP로 쓴다. 높을수록(0에 가까울수록) 제대로 발음한 것.

쓰는 데: 하드룰 오류인데 GOP가 높으면 ASR 미세오인식으로 보고 '불확실(검수
필요)'로 강등한다. 제대로 읽었는데 오답으로 잡히는 false positive를 줄이려는 것.
"""
