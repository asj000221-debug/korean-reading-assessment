"""STEP 4+ — 발달 위계 스킬맵 + 배치/처방 엔진 (학습 0).

임상 사례연구의 '치료절차'는 평면이 아니라 **위계**다.
음운인식·해독·쓰기 3영역이 병렬로, 각 영역 안에서 쉬운 것 → 어려운 것 순으로
전제조건(prerequisite)을 이루며 쌓인다. 아동이 막힌 지점(현재 발달 단계)을 찾고
다음 목표를 처방하려면 이 위계를 명시적 그래프로 인코딩해야 한다.

이 모듈은:
  1) SKILLS — 152회기 치료절차를 노드(하위기술) DAG로 인코딩
  2) placement() — 오류/정확도 증거로 '현재 발달 단계'(막힌 첫 노드) 산출
  3) prescribe() — 약점 노드 → 다음 목표 + PPT 근거 중재법(개선방향) 자동 처방

전부 규칙(학습 0). 노드 id는 error_taxonomy.py가 가리키는 약점 스킬과 일치한다.
"""
...
# 노드 = 하위기술
#   domain : '음운인식' | '해독' | '쓰기'
#   level  : 발달 순서(회기 진행 기준; 작을수록 먼저). 배치/정렬에 사용
#   label  : 한글 명칭
#   prereqs: 전제조건 노드 id 리스트(이게 숙달돼야 진입)
#   basis  : 근거 관찰/회기
#   method : 이 노드가 약할 때의 중재법(PPT 실제 방법 근거; 처방 텍스트)
...
if __name__ == "__main__":
    # 시연: 임상 사례 사례 초기(단모음 단계에서 막힘) 가정 증거
    ev = {
