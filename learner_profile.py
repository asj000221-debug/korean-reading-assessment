...
원 사례연구는 사실상 한 아동의 다회기 종단 기록(익명화)이다. 회기마다 긍정/한계를
적고 다음 개선방향을 정한다. 기존 시스템은 1회성 채점만 하는데, 진단/중재의 본질은
시간에 따른 변화라 프로파일이 있어야 한다.

회기 결과를 JSON으로 쌓아 스킬별 숙달도·오류유형 빈도·유창성(어절/분) 추이를 내고,
임상가가 손으로 쓰던 학습결과(긍정/한계)와 개선방향을 자동 생성한다.
규준(norm) 대조는 규준표가 있어야 해서 자리만 잡아뒀다.
"""
...
if __name__ == "__main__":
    # 시연: 가상 종단 기록(사례 구조를 본떴다) — profiles/_demo.json 생성
    lid = "_demo"
