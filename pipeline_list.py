"""트랙 1 — 목록(긴 발화) 읽기 파이프라인.

여러 단어를 '이어서' 읽은 1개 녹음을 인식하면, 모델이 짧은단어 사각지대를
피해 잘 인식한다(연결발화 1.5s+). 그 결과를 각 단어에 정렬해 단어별 채점.

흐름: items(순서) → 각 기대발음 → (wav 1개 인식 | dry actual) →
      align.diff_errors_by_word → scoring.score_from_errors(단어별) → 리포트
"""
